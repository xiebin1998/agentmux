...
1. **提供方检测** —— 启动时自动检测一次，确认钉钉 CLI 与 Agent CLI 都在；
   需要重检时点右上角的 **⟳**（检测是显式动作，不会实时轮询）
2. **创建项目** —— 填名称、选工作目录（可手输也可点「选择…」）、选 IM/Agent 平台
3. **启动监听** —— 在左侧**项目行内**点该项目自己的「@我」或「单聊」

「运行总览」页按 **IM 平台 / Agent 平台** 分区列出内置 CLI 及其版本与登录态。
...
│   ├── providers.tsx      提供方检测（启动时只检测一次 + 手动刷新）
│   └── components/        项目树、消息流、事件流、回复历史、监听日志、
│                          提供方检测、运行总览、设置、压缩、CLI 单选、项目对话框
